...
def draw_hud(img, mode, hand_gesture, shown, raw, d, face, hands, body, base, bg_mode="original", mirrored=True, discord_mode=False, hand_tracker=None):
    ...
    if mode == "tracking":
        ...
        lines = [
            ...
            ("Gestures: 🤟 Spiderman Web Shot  👐 Kamehameha Blast  🙌 Spirit Bomb  ⚖️ 6 7 Motion", (200, 200, 255)),
        ...
        lines = [
            ("MODE: [CLEAN PASSTHROUGH 📷]  (press 'm' to switch mode)", (0, 255, 255)),
            (f"Background: [{bg_mode.upper()}]   Mirror: [{'ON' if mirrored else 'OFF'}]   Discord Mode: [{'ON' if discord_mode else 'OFF'}]", (0, 255, 255)),
        ...
        lines = [
            ...
            ("keys: q quit  d hud  o perf  h hide/show  m mode  b bg  f mirror  t discord  c recalibrate  1-9 0 - = [ p s test", (0, 255, 0)),
        ]
...
def main():
    ...
    render_ms = 0.0
    latency_ms = 0.0
    print("Running. Focus the preview window: q quit, d HUD, o perf, m mode, b bg, f mirror, t discord, r reset, c recalibrate, 1-9 0 - = [ p s test")
